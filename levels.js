...
function generateLevel(num) {
  ...
  if (num === 30) {
    return {
      num: 30, world: 4, worldName: 'Castle', worldW: 800, theme: 4,
  ...
  const level = {
    ...
    hasEleanor: false,
    hasCastleExit: num === 29,  // level 29: little castle with Eleanor in it instead of a flag
    challengeType: CHALLENGE_TYPES[(num - 1) % CHALLENGE_TYPES.length],
    theme: world.id,
    slippery: world.id === 1,
    underwater: world.id === 2,
  };
...
function placeTube(level, rand) {
  // Tube must (1) stay >=90px before the exit so the goal is reachable
  // and (2) not have a floating platform overhead, or the player can't jump on top to enter.
  ...
  const floats = level.platforms.filter(p => p.type === 'float' || p.type === 'tree');
  for (let attempt = 0; attempt < 30; attempt++) {
    const target = candidates[Math.floor(rand() * candidates.length)];
    const tubeMaxX = exitX - 90;
    ...
    const tubeRight = tx + 50;
    const tubeTop = target.y - 42;
    // Check no floating/tree platform is within ~150px above the tube area
    const blocking = floats.some(p =>
      p.x < tubeRight + 20 &&
      p.x + p.w > tx - 20 &&
      p.y + p.h > tubeTop - 150 &&
      p.y < tubeTop
  ...
  // Couldn't find an unblocked spot — skip the tube on this level rather than block the goal.
}
